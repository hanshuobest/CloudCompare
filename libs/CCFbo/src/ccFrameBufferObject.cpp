...
void ccFrameBufferObject::reset()
{
	...
	if (m_fboId != 0)
	{
		m_glExtFunc.glDeleteFramebuffers(1, &m_fboId);//释放帧缓存对象
		m_fboId = 0;
	}
...
bool ccFrameBufferObject::init(unsigned w, unsigned h)
{
	...
	m_glExtFunc.glGenFramebuffers(1, &m_fboId);//分配帧缓存对象，并将他们存储到ids中
...
bool ccFrameBufferObject::start()
{
	...
	{
		m_glExtFunc.glBindFramebuffer(GL_FRAMEBUFFER_EXT, m_fboId);//绑定帧缓存
		return true;
	}
	else
	{
										...
										GLenum target/*=GL_TEXTURE_2D*/)
{
	...
	m_glExtFunc.glFramebufferTexture2D(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, target, texID, 0);//将二维纹理关联到帧缓存对象
	GLenum status = m_glExtFunc.glCheckFramebufferStatus(GL_FRAMEBUFFER_EXT);//测试帧缓存对象状态
