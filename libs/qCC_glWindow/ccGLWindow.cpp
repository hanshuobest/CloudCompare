...
void ccGLWindow::setStandardOrthoCenter()
{
	...
	glFunc->glOrtho(-halfW, halfW, -halfH, halfH, -maxS, maxS);//设置正交投影矩阵与当前矩阵相乘
	glFunc->glMatrixMode(GL_MODELVIEW);
	glFunc->glLoadIdentity();
}
...
void ccGLWindow::mouseMoveEvent(QMouseEvent *event)
{
	...
	if (m_interactionFlags & INTERACT_SIG_MOUSE_MOVED)//鼠标移动标记
	{
	...
	//no button pressed
	if (event->buttons() == Qt::NoButton) //没有按钮
	{
...
void ccGLWindow::glEnableSunLight()
{
	...
	glFunc->glLightModelf(GL_LIGHT_MODEL_TWO_SIDE, getDisplayParameters().lightDoubleSided ? GL_TRUE : GL_FALSE);//设置光照的模型参数
...
//draw a unit circle in a given plane (0=YZ, 1 = XZ, 2=XY) 画一个单位圆
